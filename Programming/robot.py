...
class MyRobot(wpilib.TimedRobot):
    ...
    def teleopPeriodic(self, speed_clamp=0.75):
        '''
        DRIVING CODE ==============================================================================
        '''
        if self.drivetrain_lockout == False:
            ...
            if self.controller2.getLeftStickButton() and self.leftStickCooldown >= 10:
        ...
        if self.controller2.getRightStickButton() and self.rightStickCooldown >= 10:
        ...
        if self.controller.getR2ButtonPressed() or self.rightStickHold:
            self.shooter.set_intake_position(1)
        if self.controller.getR2ButtonReleased() or self.rightStickHold:
            self.shooter.set_intake_position(0)

        # Enables/disables stage 1 in between balls
        if self.controller.getCrossButtonPressed():
            self.shooter.enable_stage1()
        if self.controller.getCrossButtonReleased():
        ...
        if self.controller.getCircleButtonReleased():
            self.shooter.disable_stage1()
        ...
        if self.controller.getR1ButtonPressed() and self.shooting == False:

            # Get current time
            current_time = time.time_ns()
            
            # Spinup shooter
            if not self.controller.getTriangleButton():
                self.shooter.enable_shooter(self.high_goal_power)
            else:
